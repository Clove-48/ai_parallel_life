...
## ⚠️ 硬规则（必须遵守，否则输出作废）
1. **只输出 JSON**，不要任何前缀/后缀文字、解释、对话、提问、问候
2. **不要向用户提问**（不要"请告诉我..."、"我需要..."、"可以告诉我..."等）
3. **不要用对话式开头**（不要"你好"、"我理解"、"当然"、"好的"等寒暄）
4. **不要 markdown 代码块围栏**（不要 ```json ... ```）
5. **不要思考过程**（不要"让我想一下..."、"首先..."、"我会..."等）
6. 即使用户提供的信息很少、很模糊，也**必须**基于已有信息生成完整 JSON
   （信息少可以写更少场景，但不能拒绝输出或反问用户）
7. 第一字符必须是 `{`，最后一字符必须是 `}`"""


# ─── JSON 抽取辅助（兼容 LLM 偶发包 markdown / 思考块 / 前言说明） ───────────


def _strip_llm_wrappers(text: str) -> str:
    """
    清理 LLM 返回内容中常见的"非 JSON 外壳"，避免正则匹配把外壳一起吞掉：
    1. 去除 <think>...</think> 思考块（DeepSeek-Reasoner 等）
    2. 去除 markdown 代码块围栏 ```json ... ```
    """
    # 1) 思考块
    text = _re_json.sub(r"<think>.*?</think>", "", text, flags=_re_json.DOTALL)
    text = _re_json.sub(r"<thinking>.*?</thinking>", "", text, flags=_re_json.DOTALL)
    # 2) markdown code fence（保留 fence 内部）
    text = _re_json.sub(r"```(?:json|JSON)?\s*", "", text)
    text = text.replace("```", "")
    return text.strip()
...
def _extract_json_object(text: str) -> str | None:
    r"""
    用括号计数找出第一个**完整**的 JSON 对象字面量。
    解决贪婪正则 \{.*\} 把"前言文字 + JSON"一起吞的 bug。
    """
    if not text:
        return None
    # 第一个 '{' 之后开始计数
...
def _wrap_text_as_freestory(buffer: str, prompt: str | None = None) -> dict:
    """
    温柔降级：把 LLM 返回的"非 JSON 文字"包装成有效的 narrative 结构。
    - 自动识别"反向提问"型（"告诉我..."、"我需要..."），加 _hint 标记
    - 文字内容当 diary 场景显示
    - 不再返回"格式异常"错误
    """
    text = (buffer or "").strip()
    if not text:
        text = "（AI 没有返回任何内容，请稍后重试）"

    # 是否在反向提问/索取信息
    question_markers = ["告诉我", "我需要", "请告诉我", "我问你", "可以告诉我",
                        "你愿意", "你愿意告诉我", "请提供", "你能否", "我想知道"]
    is_questioning = any(m in text for m in question_markers) or text.count("？") + text.count("?") >= 2
    hint = None
    if is_questioning:
        hint = ("AI 似乎在向你提问，但你已经提供了关键信息。"
                "这是 LLM 偶发不遵守格式指令的兜底。已把它的回复作为一段自由叙事展示。")

    # 用 2 个 \n 切分内容成最多 4 段
    paragraphs = [p.strip() for p in _re_json.split(r"\n{2,}", text) if p.strip()]
    if not paragraphs:
        paragraphs = [text]
    ...
    if not real_scenes:
        real_scenes.append({
            "type": "diary", "scene": "记忆回放",
            "content": text[:300], "emotion": "沉淀", "time": "那个夏天",
        })

    # 平行线：放后两段（更具想象感）
    for p in paragraphs[2:4]:
        parallel_scenes.append({
            "type": "diary",
            "scene": "另一条路",
            "content": p[:300],
            "emotion": "未知",
    ...
    if not parallel_scenes:
        parallel_scenes.append({
            "type": "diary", "scene": "另一条路",
            "content": text[:300], "emotion": "未知", "time": "如果当初",
        })

    result = {
        "title": "如果当初...",
        "realLine": real_scenes,
        "parallelLine": parallel_scenes,
        "reflection": {
            "insight": "每条路都有它自己的意义。",
            "message": "不论你当时怎么选，都已经在现在这条路上走了这么远。",
            "keywords": {
                "real": ["回忆", "沉淀", "当下", "前路"],
                "parallel": ["未知", "想象", "也许", "另一端"],
            },
        },
    }
    if hint:
...
async def _openai_stream(prompt: str) -> AsyncGenerator[dict, None]:
    ...
    try:
        ...
        async for chunk in stream:
            ...
            # 尝试完整解析 JSON（多策略）
            parsed = _try_parse_json(buffer)
            if parsed is not None:
                yield {"event": "parsed", "data": parsed}
                return
        ...
        # 失败 — 温柔降级：把 LLM 返回的文字包装成自由叙事
        # 不再让用户看到"格式异常"这种抽象错误
        import sys as _sys
        preview = buffer[:300].replace("\n", "\\n")
        print(f"[narrative] JSON 解析失败，buffer 长度={len(buffer)}，前 300 字符: {preview!r}", file=_sys.stderr, flush=True)
        fallback = _wrap_text_as_freestory(buffer, prompt)
        yield {"event": "parsed", "data": fallback}
        return
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            async with client.stream(
                ...
                import sys as _sys
                preview = buffer[:300].replace("\n", "\\n")
                print(f"[narrative][deepseek] JSON 解析失败，buffer 长度={len(buffer)}，前 300 字符: {preview!r}", file=_sys.stderr, flush=True)
                fallback = _wrap_text_as_freestory(buffer, prompt)
                yield {"event": "parsed", "data": fallback}
                return
