...
│   │   ├── pages/
│   │   │   ├── home.js            # 首页（欢迎引导）
│   │   │   ├── chat.js            # 对话采集（LLM 智能对话）
│   │   │   ├── generating.js      # 生成过渡（SSE 流式加载）
│   │   │   ├── timeline.js        # 时间线展示（双线分屏）
│   │   │   ├── card.js            # 感悟卡片（翻转/收藏/分享/反馈）
│   │   │   └── history.js         # 历史记录
│   │   ├── components/
│   │   │   ├── CardFactory.js     # 多媒介卡片渲染
...
│   │   ├── chat_service.py        # 智能对话 Prompt & 流式处理
│   │   └── narrative.py           # 叙事生成逻辑 & 降级方案
│   ├── requirements.txt
│   ├── Dockerfile
│   ├── .env.example
...
测试覆盖：健康检查 → 对话 API → 字段提取 → SSE 叙事生成 → 故事 CRUD → 反馈提交 → 404 处理。
